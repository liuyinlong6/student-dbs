<?php 
	...
	$wmim = empty($_REQUEST['daan'])?"null":$_REQUEST['daan'];
	include("conns.php");
	$responseArr = array(
			"code"=>200,
			"msg"=>"登录成功"
	...
	if (mysqli_num_rows($result)>0) {
		...
		if ($wmim == $arr['answer']) {
			...
			echo "<script> window.location.href = 'banj.php';</script>";
		}else{
			//邮件正确密码错误
			$responseArr["code"] = 20001;
			$responseArr["msg"] = "答案错误";
			// echo "<script> window.location.href = 'forget-sava-ajax.php';</script>";
		}
		// print_r($arr);
	}else{
		//邮件不存在
